...
@Component({
  selector: 'app-currentvisitdetails',
  templateUrl: './currentvisitdetails.component.html',
  styleUrls: ['./currentvisitdetails.component.scss'],
})
export class CurrentvisitdetailsComponent {
  ...
  currentVisitDetails() {
    this.service
      ...
      .subscribe((response: any) => {
        this.curentVisitDetails = response;
        console.log('current vivit details', this.curentVisitDetails);
        ...
        if (response === null) {
          ...
          sessionStorage.setItem(
            'currentVisitDetails',
            JSON.stringify(this.curentVisitDetails)
          );
        }
      });
  }
