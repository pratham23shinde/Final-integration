...
import {
  AppointmentDetails,
  VisitId,
} from './components/previoushealthrecords/previoushealthrecords.component';
...
export class NurseService {
  ...
  getAllAppointment(): Observable<AppointmentList[]> {
    return this.http.get<AppointmentList[]>(
      'http://localhost:9001/appointment-service/appointments?acceptance=Accepted'
    );
  }
  ...
  getPeviousAppointmentVisitHistory(appointmentId: number) {
    return this.http.get(
      `http://localhost:9001/appointment-service/${appointmentId}/health-records`
    );
  }
  ...
  addVisitDetails(visitDetails: VisitDetails): Observable<Object> {
    console.log(
      'in service checking type of bp',
      typeof visitDetails.bpSystolic
    );
    console.log('physicaian email', visitDetails);
    return this.http.post(
      `http://localhost:9001/patient-health-records-service/patient/health-records`,
      visitDetails
    );
  }
  ...
  getAllAllergy(): Observable<Allergy[]> {
    ...
    return this.http.get<Allergy[]>(
      'http://localhost:9001/allergy-service/allergyList'
    );
  }
  ...
  getVisitId(appointmentId: number): Observable<Object> {
    return this.http.get(
      `http://localhost:9001/patient-health-records-service/patient/visitDetails/${appointmentId}`
    );
  }
  ...
  getBloodGroup(patientId: number) {
    return this.http.get(
      `http://localhost:9001/patient-health-records-service/patient/${patientId}/bloodgroup`
    );
  }
  ...
  getDoctorsAppointment(physicianEmail: string, date: string) {
    return this.http.get(
      `http://localhost:9001/appointment-service/appointment/${physicianEmail}/${date}`
    );
  }
  ...
  getAppointmentHistoryDetailsById(patientId: any) {
    return this.http.get(
      `http://localhost:9001/appointment-service/appointment/${patientId}/previous/nurse`
    );
  }
  //munmun
  ...
  getTodaysAppointment(date: string, acceptance: string) {
    return this.http.get(
      `http://localhost:9001/appointment-service/appointment/nurse/${date}?acceptance=Accepted`
    );
  }
  ...
  getNurseDetailsByEmail(email: String) {
    return this.http.get(
      `http://localhost:9001/patient-health-records-service/nursedetails/${email}`
    );
  }
  //physician info list
  getAllDoctors(): Observable<PhysicianDetailsList[]> {
    return this.http.get<PhysicianDetailsList[]>(
      `http://localhost:9001/physician-availability-service/physician-availability`
    );
  }
  ///

  getTodaysAppointmentByDoctorEmail(
