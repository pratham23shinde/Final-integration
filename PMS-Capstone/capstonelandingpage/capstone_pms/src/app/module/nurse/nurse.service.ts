...
export class NurseService {
  ...
  getAllTests(visitId: number):Observable<TestList[]> {
    return this.http.get<TestList[]>(`http://localhost:9005/api/v1/patient/${visitId}/test-records`)
  }
...
//for previous appointment prescription health record service
  ...
  getAllAllergy(): Observable<Allergy[]>{
    ...
    return this.http.get<Allergy[]>('http://localhost:9002/api/v1/allergyList');
  }

  //for visit id from patient health record service
  getVisitId(appointmentId: number): Observable<Object>{
    return this.http.get(`http://localhost:9005/api/v1/patient/visitDetails/${appointmentId}`);
  }
